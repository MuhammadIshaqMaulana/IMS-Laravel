        ...
        <a href="{{ route('item.show', $item->id) }}" class="text-decoration-none text-dark h-100 d-flex flex-column">
            ...
            <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-1">
                    ...
                    @if($item->is_bom) <span class="badge bg-primary text-white" style="font-size: 0.55rem;">BOM</span> @endif
                </div>

                <div class="mb-2 d-flex flex-wrap gap-1 align-items-center">
                    @if(is_array($item->tags))
                        @foreach(array_slice($item->tags, 0, 2) as $tag)
                            <span class="badge bg-secondary-subtle text-secondary border small" style="font-size: 0.6rem;">#{{ $tag }}</span>
                        @endforeach
                    @endif
                    @if($item->note) <i class="fas fa-sticky-note text-warning ms-auto small shadow-sm" title="{{ $item->note }}" data-bs-toggle="tooltip"></i> @endif
                </div>
                ...
                <div class="d-flex justify-content-between align-items-end mt-2">
                    ...
                    <div class="text-end">
                        <small class="text-muted d-block" style="font-size: 0.55rem;">Beli: Rp{{ number_format($item->harga_beli, 0) }}</small>
